...
laov[["{{ iv }}"]] <- merge(get_anova_table(aov), laov[["{{ iv }}"]], by="Effect", suffixes = c("","'"))
...
pwc.long <- emmeans_test(dplyr::group_by_at(wdat.long, "{{ iv }}"),
                         {{ pivot.value }} ~ {{ pivot.key }},
                         p.adjust.method = "bonferroni")
lpwc[["{{ iv }}"]] <- merge(plyr::rbind.fill(pwc, pwc.long), lpwc[["{{ iv }}"]], by=c("{{ iv }}","term",".y.","group1","group2"), suffixes = c("","'"))
```
