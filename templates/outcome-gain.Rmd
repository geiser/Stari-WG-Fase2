---
title: "ANOVA in Gains for {{ label.en }} ({{ label.pt }})"
...
no {{ label.pt }} (medido usando a diferença entre post-test e pre-testes).
 - ANOVA test to determine whether there were significant differences
in {{ label.en }} (measured using the difference between post-test and pre-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("{{ dv }}~",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("{{ dv }}~grupo","*",x),nrow(aov)), aov))  
  }
})
