---
title: "ANCOVA in TDE - Escrita (Acertos) (TDE - Escrita (Acertos))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na TDE - Escrita (Acertos) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in TDE - Escrita (Acertos) (measured using pre- and post-tests).
...
no TDE - Escrita (Acertos) (medido usando pre- e pos-testes).
 - ANCOVA test to determine whether there were significant differences
in TDE - Escrita (Acertos) (measured using pre- and post-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("score.tde.pos~score.tde.pre+",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("score.tde.pos~score.tde.pre+grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "score.tde.pos", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = ancova.pwc(pdat, "score.tde.pos", "grupo", "score.tde.pre", pwc.covar=T)
  (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "score.tde.pos", c("grupo"), "apa-format", covar = "score.tde.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "score.tde", c("time","grupo"))
  
  pwc.long = anova.pwc(pdat.long, "score.tde", c("time","grupo"))
  pwc.long = pwc.long$score.tde$time
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaPlots(pdat, "score.tde.pos", "grupo", aov1, pwc$score.tde.pos,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat, "score.tde.pos", "grupo", aov1, pwc$score.tde.pos, covar = "score.tde.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat.long, "score.tde", "grupo", aov1, pwc.long, pre.post = "time",
...
pdat = tryCatch(remove_group_data(pdat, "score.tde.pos", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = ancova.pwc(pdat, "score.tde.pos", c("grupo","Sexo"), "score.tde.pre", pwc.covar=T)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Sexo"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "score.tde.pos", c("grupo","Sexo"), "apa-format", covar = "score.tde.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "score.tde", c("time","grupo","Sexo"))
  
  pwc.long = anova.pwc(pdat.long, "score.tde", c("time","grupo","Sexo"))
  pwc.long = pwc.long$score.tde$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Sexo"]]))
    plots <- twoWayAncovaPlots(
      pdat, "score.tde.pos", c("grupo","Sexo"), aov2, pwc$score.tde.pos,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "score.tde.pos", c("grupo","Sexo"), aov2, pwc$score.tde.pos, covar = "score.tde.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "score.tde", c("grupo","Sexo"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "score.tde.pos", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = ancova.pwc(pdat, "score.tde.pos", c("grupo","Zona"), "score.tde.pre", pwc.covar=T)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Zona"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "score.tde.pos", c("grupo","Zona"), "apa-format", covar = "score.tde.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "score.tde", c("time","grupo","Zona"))
  
  pwc.long = anova.pwc(pdat.long, "score.tde", c("time","grupo","Zona"))
  pwc.long = pwc.long$score.tde$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Zona"]]))
    plots <- twoWayAncovaPlots(
      pdat, "score.tde.pos", c("grupo","Zona"), aov2, pwc$score.tde.pos,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "score.tde.pos", c("grupo","Zona"), aov2, pwc$score.tde.pos, covar = "score.tde.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "score.tde", c("grupo","Zona"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "score.tde.pos", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = ancova.pwc(pdat, "score.tde.pos", c("grupo","Cor.Raca"), "score.tde.pre", pwc.covar=T)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Cor.Raca"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "score.tde.pos", c("grupo","Cor.Raca"), "apa-format", covar = "score.tde.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "score.tde", c("time","grupo","Cor.Raca"))
  
  pwc.long = anova.pwc(pdat.long, "score.tde", c("time","grupo","Cor.Raca"))
  pwc.long = pwc.long$score.tde$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Cor.Raca"]]))
    plots <- twoWayAncovaPlots(
      pdat, "score.tde.pos", c("grupo","Cor.Raca"), aov2, pwc$score.tde.pos,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "score.tde.pos", c("grupo","Cor.Raca"), aov2, pwc$score.tde.pos, covar = "score.tde.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "score.tde", c("grupo","Cor.Raca"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
```{r, include=FALSE}
pdat = dat[!is.na(dat[["grupo"]]) & !is.na(dat[["score.tde.quintile"]]),]
pdat = tryCatch(remove_group_data(pdat, "score.tde.pos", c("grupo","score.tde.quintile")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["score.tde.quintile"]])) > 1) {
  pwc = ancova.pwc(pdat, "score.tde.pos", c("grupo","score.tde.quintile"), "score.tde.pre", pwc.covar=T)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["score.tde.quintile"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "score.tde.pos", c("grupo","score.tde.quintile"), "apa-format", covar = "score.tde.pre"))
}
```
...
```{r, echo=FALSE, purl=FALSE}
if (!is.null(pdf) && !is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["score.tde.quintile"]])) > 1) {
  pdf <- round.pval(pdf)
  kable(pdf[,c(".y.","grupo","score.tde.quintile","group1","group2","estimate","conf.low",
               "conf.high","se","statistic","p","p.adj","p.adj.signif")], digits = 3)
}
```
...
```{r, include=FALSE}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["score.tde.quintile"]])) > 1) {
  pdat.long = dat.long[!is.na(dat.long[["grupo"]]) & !is.na(dat.long[["score.tde.quintile"]]),]
  pdat.long = remove_group_data(pdat.long, "score.tde", c("time","grupo","score.tde.quintile"))
  
  pwc.long = anova.pwc(pdat.long, "score.tde", c("time","grupo","score.tde.quintile"))
  pwc.long = pwc.long$score.tde$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["score.tde.quintile"]])) > 1) {
  pdf.long <- round.pval(pdf.long)
  kable(pdf.long[,c(".y.","grupo","score.tde.quintile","group1","group2","estimate","conf.low",
                    "conf.high","se","statistic","p","p.adj","p.adj.signif")], digits = 3)
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["score.tde.quintile"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["score.tde.quintile"]]))
    plots <- twoWayAncovaPlots(
      pdat, "score.tde.pos", c("grupo","score.tde.quintile"), aov2, pwc$score.tde.pos,
      addParam = c("mean_se"), font.label.size=10,
      step.increase=0.05, subtitle = which(aov2$Effect == "grupo:score.tde.quintile"))
```
...
```{r, dpi=300, fig.width=7, fig.height=7}
if (!is.null(pdat) && !is.null(nrow(plots[["score.tde.quintile"]]$data)))
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["score.tde.quintile"]]))
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["score.tde.quintile"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["score.tde.quintile"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "score.tde.pos", c("grupo","score.tde.quintile"), aov2, pwc$score.tde.pos, covar = "score.tde.pre",
      theme = "classic", color = color[["grupo:score.tde.quintile"]],
      subtitle = which(aov2$Effect == "grupo:score.tde.quintile"))
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["score.tde.quintile"]])) > 1)
  if (!is.null(pwc[["score.tde.pos"]][["grupo"]]) || !is.null(pwc[["score.tde.pos"]][["score.tde.quintile"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "score.tde", c("grupo","score.tde.quintile"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
