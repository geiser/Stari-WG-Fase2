---
title: "ANOVA in Gains for gain flow (prod. textual) (gain flow (prod. textual))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na gain flow (prod. textual) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in gain flow (prod. textual) (measured using pre- and post-tests).
...
no gain flow (prod. textual) (medido usando a diferença entre post-test e pre-testes).
 - ANOVA test to determine whether there were significant differences
in gain flow (prod. textual) (measured using the difference between post-test and pre-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("gain.flow.text~",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("gain.flow.text~grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.text", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.text", "grupo")
  (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.text", c("grupo"), "apa-format"))
}
```
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAnovaPlots(pdat, "gain.flow.text", "grupo", aov1, pwc$gain.flow.text,
...
#    pdat, "gain.flow.text", "grupo", aov1, pwc$gain.flow.text,",
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (prod. textual)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots <- oneWayAncovaBoxPlots(
#    pdat.long, "gain.flow.text", "grupo", aov1, pwc.long, pre.post = "time",
#    theme = "classic", color = color$prepost)
```

```{r, dpi=300, fig.width=10, fig.height=6}
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (prod. textual)")
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.text", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.text", c("grupo","Sexo"))
  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Sexo"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.text", c("grupo","Sexo"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Sexo"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.text", c("grupo","Sexo"), aov2, pwc$gain.flow.text,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
#  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Sexo"]]))
#    plots[["grupo:Sexo"]] + ggplot2::ylab("gain flow (prod. textual)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.text", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.text", c("grupo","Zona"))
  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Zona"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.text", c("grupo","Zona"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Zona"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.text", c("grupo","Zona"), aov2, pwc$gain.flow.text,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
#  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Zona"]]))
#    plots[["grupo:Zona"]] + ggplot2::ylab("gain flow (prod. textual)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.text", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.text", c("grupo","Cor.Raca"))
  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Cor.Raca"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.text", c("grupo","Cor.Raca"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Cor.Raca"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.text", c("grupo","Cor.Raca"), aov2, pwc$gain.flow.text,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
#  if (!is.null(pwc[["gain.flow.text"]][["grupo"]]) || !is.null(pwc[["gain.flow.text"]][["Cor.Raca"]]))
#    plots[["grupo:Cor.Raca"]] + ggplot2::ylab("gain flow (prod. textual)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
