...
---
title: "ANCOVA in Reading Correct Words (1 Min) (Reading Correct Words (1 Min))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na Reading Correct Words (1 Min) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in Reading Correct Words (1 Min) (measured using pre- and post-tests).
...
no Reading Correct Words (1 Min) (medido usando pre- e pos-testes).
 - ANCOVA test to determine whether there were significant differences
in Reading Correct Words (1 Min) (measured using pre- and post-tests).
...
saveRDS(
  list(
    test = "ancova",
    dv = "TFL.corretas.per.min.pos",
    covar = "TFL.corretas.per.min.pre",
    iv = c("grupo"),
    wid = "id",
    transformation = "none",
  ...
  file = 'Stari.Grupo-ancova-TFLcorretasperminpos-from-TFLcorretasperminpre-grupo.rds')
```
...
saveRDS(
  ...
  file = 'Stari.Grupo-ancova-TFLcorretasperminpos-from-TFLcorretasperminpre-grupo-Sexo.rds')
```
...
saveRDS(
  ...
  file = 'Stari.Grupo-ancova-TFLcorretasperminpos-from-TFLcorretasperminpre-grupo-Zona.rds')
```
...
saveRDS(
  ...
  file = 'Stari.Grupo-ancova-TFLcorretasperminpos-from-TFLcorretasperminpre-grupo-CorRaca.rds')
```
...
saveRDS(
  ...
  file = 'Stari.Grupo-ancova-TFLcorretasperminpos-from-TFLcorretasperminpre-grupo-Serie.rds')
```
...
saveRDS(
  list(
    ...
    dv = "TFL.corretas.per.min.pos",
    covar = "TFL.corretas.per.min.pre",
    iv = c("grupo","TFL.corretas.per.min.quintile"),
    wid = "id",
    transformation = "none",
    outliers = list(method = "none"),
    non.normality = non.normal,
  ...
  file = 'Stari.Grupo-ancova-TFLcorretasperminpos-from-TFLcorretasperminpre-grupo-TFLcorretasperminquintile.rds')
```
...
```{r include=FALSE}
### Saving ancova information
saveRDS(df, file = '/mnt/c/Users/geise/OneDrive/Workspace/Stari-WG-Fase2/code/aov-stari-TFL.corretas.per.min-emms.rds')
```
