...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("fss.media.text~dfs.media.text+",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("fss.media.text~dfs.media.text+grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.text", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.text", "grupo", "dfs.media.text", pwc.covar=T)
  (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.text", c("grupo"), "apa-format", covar = "dfs.media.text"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.text", c("time","grupo"))
  
  pwc.long = anova.pwc(pdat.long, "flow.text", c("time","grupo"))
  pwc.long = pwc.long$flow.text$time
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaPlots(pdat, "fss.media.text", "grupo", aov1, pwc$fss.media.text,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat, "fss.media.text", "grupo", aov1, pwc$fss.media.text, covar = "dfs.media.text",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat.long, "flow.text", "grupo", aov1, pwc.long, pre.post = "time",
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.text", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.text", c("grupo","Sexo"), "dfs.media.text", pwc.covar=T)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Sexo"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.text", c("grupo","Sexo"), "apa-format", covar = "dfs.media.text"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.text", c("time","grupo","Sexo"))
  
  pwc.long = anova.pwc(pdat.long, "flow.text", c("time","grupo","Sexo"))
  pwc.long = pwc.long$flow.text$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Sexo"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.text", c("grupo","Sexo"), aov2, pwc$fss.media.text,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.text", c("grupo","Sexo"), aov2, pwc$fss.media.text, covar = "dfs.media.text",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.text", c("grupo","Sexo"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.text", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.text", c("grupo","Zona"), "dfs.media.text", pwc.covar=T)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Zona"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.text", c("grupo","Zona"), "apa-format", covar = "dfs.media.text"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.text", c("time","grupo","Zona"))
  
  pwc.long = anova.pwc(pdat.long, "flow.text", c("time","grupo","Zona"))
  pwc.long = pwc.long$flow.text$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Zona"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.text", c("grupo","Zona"), aov2, pwc$fss.media.text,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.text", c("grupo","Zona"), aov2, pwc$fss.media.text, covar = "dfs.media.text",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.text", c("grupo","Zona"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.text", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.text", c("grupo","Cor.Raca"), "dfs.media.text", pwc.covar=T)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Cor.Raca"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.text", c("grupo","Cor.Raca"), "apa-format", covar = "dfs.media.text"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.text", c("time","grupo","Cor.Raca"))
  
  pwc.long = anova.pwc(pdat.long, "flow.text", c("time","grupo","Cor.Raca"))
  pwc.long = pwc.long$flow.text$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Cor.Raca"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.text", c("grupo","Cor.Raca"), aov2, pwc$fss.media.text,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.text", c("grupo","Cor.Raca"), aov2, pwc$fss.media.text, covar = "dfs.media.text",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.text"]][["grupo"]]) || !is.null(pwc[["fss.media.text"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.text", c("grupo","Cor.Raca"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
