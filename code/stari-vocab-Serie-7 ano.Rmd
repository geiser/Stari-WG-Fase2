---
title: "ANCOVA in Vocabulario (acertos) (Vocabulario (acertos))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na Vocabulario (acertos) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in Vocabulario (acertos) (measured using pre- and post-tests).
...
no Vocabulario (acertos) (medido usando pre- e pos-testes).
 - ANCOVA test to determine whether there were significant differences
in Vocabulario (acertos) (measured using pre- and post-tests).
...
fatores2 <- c("Sexo","Zona","Cor.Raca","vocab.quintile")
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("vocab.pos~vocab.pre+",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("vocab.pos~vocab.pre+grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "vocab.pos", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = ancova.pwc(pdat, "vocab.pos", "grupo", "vocab.pre", pwc.covar=T)
  (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "vocab.pos", c("grupo"), "apa-format", covar = "vocab.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "vocab", c("time","grupo"))
  
  pwc.long = anova.pwc(pdat.long, "vocab", c("time","grupo"))
  pwc.long = pwc.long$vocab$time
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaPlots(pdat, "vocab.pos", "grupo", aov1, pwc$vocab.pos,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat, "vocab.pos", "grupo", aov1, pwc$vocab.pos, covar = "vocab.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat.long, "vocab", "grupo", aov1, pwc.long, pre.post = "time",
...
pdat = tryCatch(remove_group_data(pdat, "vocab.pos", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = ancova.pwc(pdat, "vocab.pos", c("grupo","Sexo"), "vocab.pre", pwc.covar=T)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Sexo"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "vocab.pos", c("grupo","Sexo"), "apa-format", covar = "vocab.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "vocab", c("time","grupo","Sexo"))
  
  pwc.long = anova.pwc(pdat.long, "vocab", c("time","grupo","Sexo"))
  pwc.long = pwc.long$vocab$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Sexo"]]))
    plots <- twoWayAncovaPlots(
      pdat, "vocab.pos", c("grupo","Sexo"), aov2, pwc$vocab.pos,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "vocab.pos", c("grupo","Sexo"), aov2, pwc$vocab.pos, covar = "vocab.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "vocab", c("grupo","Sexo"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "vocab.pos", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = ancova.pwc(pdat, "vocab.pos", c("grupo","Zona"), "vocab.pre", pwc.covar=T)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Zona"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "vocab.pos", c("grupo","Zona"), "apa-format", covar = "vocab.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "vocab", c("time","grupo","Zona"))
  
  pwc.long = anova.pwc(pdat.long, "vocab", c("time","grupo","Zona"))
  pwc.long = pwc.long$vocab$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Zona"]]))
    plots <- twoWayAncovaPlots(
      pdat, "vocab.pos", c("grupo","Zona"), aov2, pwc$vocab.pos,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "vocab.pos", c("grupo","Zona"), aov2, pwc$vocab.pos, covar = "vocab.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "vocab", c("grupo","Zona"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "vocab.pos", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = ancova.pwc(pdat, "vocab.pos", c("grupo","Cor.Raca"), "vocab.pre", pwc.covar=T)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Cor.Raca"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "vocab.pos", c("grupo","Cor.Raca"), "apa-format", covar = "vocab.pre"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "vocab", c("time","grupo","Cor.Raca"))
  
  pwc.long = anova.pwc(pdat.long, "vocab", c("time","grupo","Cor.Raca"))
  pwc.long = pwc.long$vocab$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Cor.Raca"]]))
    plots <- twoWayAncovaPlots(
      pdat, "vocab.pos", c("grupo","Cor.Raca"), aov2, pwc$vocab.pos,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "vocab.pos", c("grupo","Cor.Raca"), aov2, pwc$vocab.pos, covar = "vocab.pre",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "vocab", c("grupo","Cor.Raca"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
```{r, include=FALSE}
pdat = dat[!is.na(dat[["grupo"]]) & !is.na(dat[["vocab.quintile"]]),]
pdat = tryCatch(remove_group_data(pdat, "vocab.pos", c("grupo","vocab.quintile")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["vocab.quintile"]])) > 1) {
  pwc = ancova.pwc(pdat, "vocab.pos", c("grupo","vocab.quintile"), "vocab.pre", pwc.covar=T)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["vocab.quintile"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "vocab.pos", c("grupo","vocab.quintile"), "apa-format", covar = "vocab.pre"))
}
```
...
```{r, echo=FALSE, purl=FALSE}
if (!is.null(pdf) && !is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["vocab.quintile"]])) > 1) {
  pdf <- round.pval(pdf)
  kable(pdf[,c(".y.","grupo","vocab.quintile","group1","group2","estimate","conf.low",
               "conf.high","se","statistic","p","p.adj","p.adj.signif")], digits = 3)
}
```
...
```{r, include=FALSE}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["vocab.quintile"]])) > 1) {
  pdat.long = dat.long[!is.na(dat.long[["grupo"]]) & !is.na(dat.long[["vocab.quintile"]]),]
  pdat.long = remove_group_data(pdat.long, "vocab", c("time","grupo","vocab.quintile"))
  
  pwc.long = anova.pwc(pdat.long, "vocab", c("time","grupo","vocab.quintile"))
  pwc.long = pwc.long$vocab$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["vocab.quintile"]])) > 1) {
  pdf.long <- round.pval(pdf.long)
  kable(pdf.long[,c(".y.","grupo","vocab.quintile","group1","group2","estimate","conf.low",
                    "conf.high","se","statistic","p","p.adj","p.adj.signif")], digits = 3)
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["vocab.quintile"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["vocab.quintile"]]))
    plots <- twoWayAncovaPlots(
      pdat, "vocab.pos", c("grupo","vocab.quintile"), aov2, pwc$vocab.pos,
      addParam = c("mean_se"), font.label.size=10,
      step.increase=0.05, subtitle = which(aov2$Effect == "grupo:vocab.quintile"))
```
...
```{r, dpi=300, fig.width=7, fig.height=7}
if (!is.null(pdat) && !is.null(nrow(plots[["vocab.quintile"]]$data)))
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["vocab.quintile"]]))
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["vocab.quintile"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["vocab.quintile"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "vocab.pos", c("grupo","vocab.quintile"), aov2, pwc$vocab.pos, covar = "vocab.pre",
      theme = "classic", color = color[["grupo:vocab.quintile"]],
      subtitle = which(aov2$Effect == "grupo:vocab.quintile"))
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["vocab.quintile"]])) > 1)
  if (!is.null(pwc[["vocab.pos"]][["grupo"]]) || !is.null(pwc[["vocab.pos"]][["vocab.quintile"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "vocab", c("grupo","vocab.quintile"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
