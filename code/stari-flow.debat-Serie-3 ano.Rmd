...
no flow (debate) (medido usando pre- e pos-testes).
 - ANCOVA test to determine whether there were significant differences
in flow (debate) (measured using pre- and post-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("fss.media.debat~dfs.media.debat+",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("fss.media.debat~dfs.media.debat+grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.debat", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.debat", "grupo", "dfs.media.debat", pwc.covar=T)
  (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.debat", c("grupo"), "apa-format", covar = "dfs.media.debat"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.debat", c("time","grupo"))
  
  pwc.long = anova.pwc(pdat.long, "flow.debat", c("time","grupo"))
  pwc.long = pwc.long$flow.debat$time
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaPlots(pdat, "fss.media.debat", "grupo", aov1, pwc$fss.media.debat,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat, "fss.media.debat", "grupo", aov1, pwc$fss.media.debat, covar = "dfs.media.debat",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat.long, "flow.debat", "grupo", aov1, pwc.long, pre.post = "time",
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.debat", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.debat", c("grupo","Sexo"), "dfs.media.debat", pwc.covar=T)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Sexo"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.debat", c("grupo","Sexo"), "apa-format", covar = "dfs.media.debat"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.debat", c("time","grupo","Sexo"))
  
  pwc.long = anova.pwc(pdat.long, "flow.debat", c("time","grupo","Sexo"))
  pwc.long = pwc.long$flow.debat$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Sexo"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.debat", c("grupo","Sexo"), aov2, pwc$fss.media.debat,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.debat", c("grupo","Sexo"), aov2, pwc$fss.media.debat, covar = "dfs.media.debat",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.debat", c("grupo","Sexo"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.debat", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.debat", c("grupo","Zona"), "dfs.media.debat", pwc.covar=T)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Zona"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.debat", c("grupo","Zona"), "apa-format", covar = "dfs.media.debat"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.debat", c("time","grupo","Zona"))
  
  pwc.long = anova.pwc(pdat.long, "flow.debat", c("time","grupo","Zona"))
  pwc.long = pwc.long$flow.debat$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Zona"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.debat", c("grupo","Zona"), aov2, pwc$fss.media.debat,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.debat", c("grupo","Zona"), aov2, pwc$fss.media.debat, covar = "dfs.media.debat",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.debat", c("grupo","Zona"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.debat", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.debat", c("grupo","Cor.Raca"), "dfs.media.debat", pwc.covar=T)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Cor.Raca"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.debat", c("grupo","Cor.Raca"), "apa-format", covar = "dfs.media.debat"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.debat", c("time","grupo","Cor.Raca"))
  
  pwc.long = anova.pwc(pdat.long, "flow.debat", c("time","grupo","Cor.Raca"))
  pwc.long = pwc.long$flow.debat$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Cor.Raca"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.debat", c("grupo","Cor.Raca"), aov2, pwc$fss.media.debat,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.debat", c("grupo","Cor.Raca"), aov2, pwc$fss.media.debat, covar = "dfs.media.debat",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.debat"]][["grupo"]]) || !is.null(pwc[["fss.media.debat"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.debat", c("grupo","Cor.Raca"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
