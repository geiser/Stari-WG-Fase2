---
title: "ANOVA in Gains for gain flow (prob. matemática) (gain flow (prob. matemática))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na gain flow (prob. matemática) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in gain flow (prob. matemática) (measured using pre- and post-tests).
...
no gain flow (prob. matemática) (medido usando a diferença entre post-test e pre-testes).
 - ANOVA test to determine whether there were significant differences
in gain flow (prob. matemática) (measured using the difference between post-test and pre-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("gain.flow.math~",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("gain.flow.math~grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.math", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.math", "grupo")
  (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.math", c("grupo"), "apa-format"))
}
```
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAnovaPlots(pdat, "gain.flow.math", "grupo", aov1, pwc$gain.flow.math,
...
#    pdat, "gain.flow.math", "grupo", aov1, pwc$gain.flow.math,",
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (prob. matemática)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots <- oneWayAncovaBoxPlots(
#    pdat.long, "gain.flow.math", "grupo", aov1, pwc.long, pre.post = "time",
#    theme = "classic", color = color$prepost)
```

```{r, dpi=300, fig.width=10, fig.height=6}
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (prob. matemática)")
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.math", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.math", c("grupo","Sexo"))
  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Sexo"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.math", c("grupo","Sexo"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Sexo"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.math", c("grupo","Sexo"), aov2, pwc$gain.flow.math,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
#  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Sexo"]]))
#    plots[["grupo:Sexo"]] + ggplot2::ylab("gain flow (prob. matemática)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.math", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.math", c("grupo","Zona"))
  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Zona"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.math", c("grupo","Zona"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Zona"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.math", c("grupo","Zona"), aov2, pwc$gain.flow.math,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
#  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Zona"]]))
#    plots[["grupo:Zona"]] + ggplot2::ylab("gain flow (prob. matemática)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.math", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.math", c("grupo","Cor.Raca"))
  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Cor.Raca"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.math", c("grupo","Cor.Raca"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Cor.Raca"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.math", c("grupo","Cor.Raca"), aov2, pwc$gain.flow.math,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
#  if (!is.null(pwc[["gain.flow.math"]][["grupo"]]) || !is.null(pwc[["gain.flow.math"]][["Cor.Raca"]]))
#    plots[["grupo:Cor.Raca"]] + ggplot2::ylab("gain flow (prob. matemática)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
