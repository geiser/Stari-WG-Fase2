---
title: "ANOVA in Gains for gain flow (debate) (gain flow (debate))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na gain flow (debate) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in gain flow (debate) (measured using pre- and post-tests).
...
no gain flow (debate) (medido usando a diferença entre post-test e pre-testes).
 - ANOVA test to determine whether there were significant differences
in gain flow (debate) (measured using the difference between post-test and pre-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("gain.flow.debat~",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("gain.flow.debat~grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.debat", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.debat", "grupo")
  (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.debat", c("grupo"), "apa-format"))
}
```
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAnovaPlots(pdat, "gain.flow.debat", "grupo", aov1, pwc$gain.flow.debat,
...
#    pdat, "gain.flow.debat", "grupo", aov1, pwc$gain.flow.debat,",
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (debate)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots <- oneWayAncovaBoxPlots(
#    pdat.long, "gain.flow.debat", "grupo", aov1, pwc.long, pre.post = "time",
#    theme = "classic", color = color$prepost)
```

```{r, dpi=300, fig.width=10, fig.height=6}
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (debate)")
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.debat", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.debat", c("grupo","Sexo"))
  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Sexo"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.debat", c("grupo","Sexo"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Sexo"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.debat", c("grupo","Sexo"), aov2, pwc$gain.flow.debat,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
#  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Sexo"]]))
#    plots[["grupo:Sexo"]] + ggplot2::ylab("gain flow (debate)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.debat", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.debat", c("grupo","Zona"))
  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Zona"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.debat", c("grupo","Zona"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Zona"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.debat", c("grupo","Zona"), aov2, pwc$gain.flow.debat,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
#  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Zona"]]))
#    plots[["grupo:Zona"]] + ggplot2::ylab("gain flow (debate)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.debat", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.debat", c("grupo","Cor.Raca"))
  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Cor.Raca"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.debat", c("grupo","Cor.Raca"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Cor.Raca"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.debat", c("grupo","Cor.Raca"), aov2, pwc$gain.flow.debat,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
#  if (!is.null(pwc[["gain.flow.debat"]][["grupo"]]) || !is.null(pwc[["gain.flow.debat"]][["Cor.Raca"]]))
#    plots[["grupo:Cor.Raca"]] + ggplot2::ylab("gain flow (debate)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
