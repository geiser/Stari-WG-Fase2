---
title: "ANCOVA in flow (prob. matemática) (flow (prob. matemática))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na flow (prob. matemática) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in flow (prob. matemática) (measured using pre- and post-tests).
...
no flow (prob. matemática) (medido usando pre- e pos-testes).
 - ANCOVA test to determine whether there were significant differences
in flow (prob. matemática) (measured using pre- and post-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("fss.media.math~dfs.media.math+",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("fss.media.math~dfs.media.math+grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.math", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.math", "grupo", "dfs.media.math", pwc.covar=T)
  (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.math", c("grupo"), "apa-format", covar = "dfs.media.math"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.math", c("time","grupo"))
  
  pwc.long = anova.pwc(pdat.long, "flow.math", c("time","grupo"))
  pwc.long = pwc.long$flow.math$time
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaPlots(pdat, "fss.media.math", "grupo", aov1, pwc$fss.media.math,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat, "fss.media.math", "grupo", aov1, pwc$fss.media.math, covar = "dfs.media.math",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAncovaBoxPlots(
    pdat.long, "flow.math", "grupo", aov1, pwc.long, pre.post = "time",
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.math", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.math", c("grupo","Sexo"), "dfs.media.math", pwc.covar=T)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Sexo"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.math", c("grupo","Sexo"), "apa-format", covar = "dfs.media.math"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.math", c("time","grupo","Sexo"))
  
  pwc.long = anova.pwc(pdat.long, "flow.math", c("time","grupo","Sexo"))
  pwc.long = pwc.long$flow.math$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Sexo"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.math", c("grupo","Sexo"), aov2, pwc$fss.media.math,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.math", c("grupo","Sexo"), aov2, pwc$fss.media.math, covar = "dfs.media.math",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Sexo"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.math", c("grupo","Sexo"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.math", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.math", c("grupo","Zona"), "dfs.media.math", pwc.covar=T)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Zona"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.math", c("grupo","Zona"), "apa-format", covar = "dfs.media.math"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.math", c("time","grupo","Zona"))
  
  pwc.long = anova.pwc(pdat.long, "flow.math", c("time","grupo","Zona"))
  pwc.long = pwc.long$flow.math$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Zona"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.math", c("grupo","Zona"), aov2, pwc$fss.media.math,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.math", c("grupo","Zona"), aov2, pwc$fss.media.math, covar = "dfs.media.math",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Zona"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.math", c("grupo","Zona"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
...
pdat = tryCatch(remove_group_data(pdat, "fss.media.math", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = ancova.pwc(pdat, "fss.media.math", c("grupo","Cor.Raca"), "dfs.media.math", pwc.covar=T)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Cor.Raca"]]))
    (apa <- get.ancova.emmeans.with.ds(pwc, pdat, "fss.media.math", c("grupo","Cor.Raca"), "apa-format", covar = "dfs.media.math"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  ...
  pdat.long = remove_group_data(pdat.long, "flow.math", c("time","grupo","Cor.Raca"))
  
  pwc.long = anova.pwc(pdat.long, "flow.math", c("time","grupo","Cor.Raca"))
  pwc.long = pwc.long$flow.math$time
  (pdf.long <- pwc.long[!is.na(pwc.long$p.adj),])
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Cor.Raca"]]))
    plots <- twoWayAncovaPlots(
      pdat, "fss.media.math", c("grupo","Cor.Raca"), aov2, pwc$fss.media.math,
      addParam = c("mean_se"), font.label.size=10,
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat, "fss.media.math", c("grupo","Cor.Raca"), aov2, pwc$fss.media.math, covar = "dfs.media.math",
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["fss.media.math"]][["grupo"]]) || !is.null(pwc[["fss.media.math"]][["Cor.Raca"]]))
    plots <- twoWayAncovaBoxPlots(
      pdat.long, "flow.math", c("grupo","Cor.Raca"), aov2, pwc.long, pre.post = "time",
      theme = "classic", color = color$prepost)
```
