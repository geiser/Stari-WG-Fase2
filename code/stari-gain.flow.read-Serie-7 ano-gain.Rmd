---
title: "ANOVA in Gains for gain flow (ativ. leitura) (gain flow (ativ. leitura))"
author: Geiser C. Challco <geiser@alumni.usp.br>
comment: Teste ANCOVA para determinar se houve diferenças significativas
         na gain flow (ativ. leitura) (medido usando pre- e pos-testes).
         
         ANCOVA test to determine whether there were significant differences
         in gain flow (ativ. leitura) (measured using pre- and post-tests).
...
no gain flow (ativ. leitura) (medido usando a diferença entre post-test e pre-testes).
 - ANOVA test to determine whether there were significant differences
in gain flow (ativ. leitura) (measured using the difference between post-test and pre-tests).
...
aovs1 <- lapply(lfatores1, FUN = function(x) {
  ...
  if (!is.null(aov))
    cbind(formula=rep(paste0("gain.flow.read~",x),nrow(aov)),aov)
})
...
aovs2 <- lapply(lfatores2, FUN = function(x) {
  ...
  if (length(unique(tdat$grupo)) >= 2 &&  length(unique(tdat[[x]])) >= 2) {
    ...
    if (!is.null(aov))
      return(cbind(formula=rep(paste0("gain.flow.read~grupo","*",x),nrow(aov)), aov))  
  }
})
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.read", c("grupo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.read", "grupo")
  (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.read", c("grupo"), "apa-format"))
}
```
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
  plots <- oneWayAnovaPlots(pdat, "gain.flow.read", "grupo", aov1, pwc$gain.flow.read,
...
#    pdat, "gain.flow.read", "grupo", aov1, pwc$gain.flow.read,",
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (ativ. leitura)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots <- oneWayAncovaBoxPlots(
#    pdat.long, "gain.flow.read", "grupo", aov1, pwc.long, pre.post = "time",
#    theme = "classic", color = color$prepost)
```

```{r, dpi=300, fig.width=10, fig.height=6}
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1)
#  plots[["grupo"]] + ggplot2::ylab("gain flow (ativ. leitura)")
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.read", c("grupo","Sexo")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.read", c("grupo","Sexo"))
  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Sexo"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.read", c("grupo","Sexo"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Sexo"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.read", c("grupo","Sexo"), aov2, pwc$gain.flow.read,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Sexo"]])) > 1)
#  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Sexo"]]))
#    plots[["grupo:Sexo"]] + ggplot2::ylab("gain flow (ativ. leitura)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.read", c("grupo","Zona")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.read", c("grupo","Zona"))
  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Zona"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.read", c("grupo","Zona"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Zona"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.read", c("grupo","Zona"), aov2, pwc$gain.flow.read,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Zona"]])) > 1)
#  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Zona"]]))
#    plots[["grupo:Zona"]] + ggplot2::ylab("gain flow (ativ. leitura)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
...
pdat = tryCatch(remove_group_data(pdat, "gain.flow.read", c("grupo","Cor.Raca")), error = function(e) NULL)

if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1) {
  pwc = anova.pwc(pdat, "gain.flow.read", c("grupo","Cor.Raca"))
  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Cor.Raca"]]))
    (apa <- get.anova.emmeans.with.ds(pwc, pdat, "gain.flow.read", c("grupo","Cor.Raca"), "apa-format"))
}
```
...
if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Cor.Raca"]]))
    plots <- twoWayAnovaPlots(
      pdat, "gain.flow.read", c("grupo","Cor.Raca"), aov2, pwc$gain.flow.read,
      addParam = c("mean_se"), font.label.size=10,
...
#if (!is.null(pdat) && length(unique(pdat[["grupo"]])) > 1 && length(unique(pdat[["Cor.Raca"]])) > 1)
#  if (!is.null(pwc[["gain.flow.read"]][["grupo"]]) || !is.null(pwc[["gain.flow.read"]][["Cor.Raca"]]))
#    plots[["grupo:Cor.Raca"]] + ggplot2::ylab("gain flow (ativ. leitura)") + ggplot2::scale_x_discrete(labels=c('pre', 'pos'))
```
